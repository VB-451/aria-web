...
export default function Chat() {
    ...
    const [messages, setMessages] = useState<{ role: string; content: string; step1_decision?: {function: string} }[]>([]);
    ...
    const handleSendMessage = async (text?: string) => {
        ...
        setLoading(true);
        setShowShortCuts(false)
        const data = await sendMessage(messageToSend, "ask");
        console.log(data)
        setLoading(false);
        setMessages([...newMessages, { role: "assistant", content: data.reply, step1_decision: data.step1_decision }]);
    }
    ...
    return (
        <div id="chat" className="w-full h-screen overflow-hidden z-2">
            <div className="h-[25px] bg-linear-to-b from-black" />
